trigger WFRActions on Opportunity (before insert, before update) {
    ...
    for(Opportunity opp: Trigger.new){
        ...
        (Trigger.oldMap.get(opp.Id).New_FRSA_Has_Been_Received__c != opp.New_FRSA_Has_Been_Received__c) || 
        ...
        if(opp.Submit_to_Sales_Assistant__c != null && opp.X3_Minute_Application__c == true && opp.Legible_4_months_processing_statements__c == true
            && opp.Bank_Statements__c == true && opp.Voided_Check_Received__c == true && opp.Calculator__c == true && opp.MRA_3_Minute_are_complete__c == true
        ...
        if(Trigger.isInsert 
             && ((opp.Auto_Decisioned_By__c != null && opp.Underwriter_Assigned__c == 'AUTO DECISION')
             || (opp.Underwriter_Assigned__c != null))){
            opp.UW_Assigned_Date_Stamp__c = Datetime.now();      
        }
        
        if(Trigger.isUpdate
            && (((Trigger.oldMap.get(opp.Id).Auto_Decisioned_By__c == null) && (opp.Auto_Decisioned_By__c != null)
